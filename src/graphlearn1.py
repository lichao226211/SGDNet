...
class GraphLearner(nn.Module):
    def __init__(self, input_size, hidden_size,n_nodes,n_class,n_anchors, topk=None, epsilon=None, num_pers=16, metric_type='weighted_cosine', device=None):
        ...
        self.weight_tensor = torch.DoubleTensor(num_pers, input_size)
        self.weight_tensor = nn.Parameter(nn.init.xavier_uniform_(self.weight_tensor))

        self.weight_tensor_for_pe = torch.DoubleTensor(self.n_anchors, hidden_size)
        self.weight_tensor_for_pe = nn.Parameter(nn.init.xavier_uniform_(self.weight_tensor_for_pe)).to(self.device)
    ...
    def forward(self, context, position_encoding, gpr_rank, position_flag, ctx_mask=None):
        ...
        if self.metric_type == 'weighted_cosine':
            ...
            expand_weight_tensor = expand_weight_tensor.detach().cpu()
            ...
            markoff_value = 0
            if position_flag == 1:
                position_encoding = position_encoding.detach().cpu()
                gpr_rank = gpr_rank.detach().cpu()
    ...
    def build_epsilon_neighbourhood(self, attention, epsilon, markoff_value):
        mask = (attention > epsilon).detach().double()
